...
try {
    ...
    $config1 = Join-Path $testRoot 'profiles\existing\user.cfg'
    New-Item -ItemType Directory -Path $game1,(Split-Path -Parent $config1) -Force | Out-Null
    ...
    [IO.File]::WriteAllText($config1, $before1, (New-Object Text.UTF8Encoding($false)))
    & (Join-Path $PackageRoot 'Install-Metro2033ReduxVR.ps1') -GamePath $game1 -UserConfigPath $config1 -NonInteractive
    $installed1 = Read-All $config1
    ...
    [IO.File]::WriteAllText($config1, $changed1, (New-Object Text.UTF8Encoding($false)))
    & (Join-Path $PackageRoot 'Uninstall-Metro2033ReduxVR.ps1') -GamePath $game1 -NonInteractive
    $uninstalled1 = Read-All $config1
    ...
    $config2 = Join-Path $testRoot 'profiles\missing\user.cfg'
    ...
    & (Join-Path $PackageRoot 'Install-Metro2033ReduxVR.ps1') -GamePath $game2 -UserConfigPath $config2 -NonInteractive
    [IO.File]::AppendAllText($config2, "custom_test_value 7`n", (New-Object Text.UTF8Encoding($false)))
    & (Join-Path $PackageRoot 'Uninstall-Metro2033ReduxVR.ps1') -GamePath $game2 -NonInteractive
    $uninstalled2 = Read-All $config2
    ...
    $localAppData3 = Join-Path $testRoot 'epic-local-app-data'
    $config3 = Join-Path $localAppData3 '4A Games\Metro 2033\epic-profile\user.cfg'
    New-Item -ItemType Directory -Path $game3,$epicManifests,(Split-Path -Parent $config3) -Force | Out-Null
    Copy-Item -LiteralPath $MetroExe -Destination (Join-Path $game3 'metro.exe')
    [IO.File]::WriteAllText($config3, "r_quality_level 2`r`n", (New-Object Text.UTF8Encoding($false)))
    [ordered]@{
    ...
    $steamRoot4 = Join-Path $testRoot 'steam-root'
    $game4 = Join-Path $steamRoot4 'steamapps\common\Metro 2033 Redux'
    $localAppData4 = Join-Path $testRoot 'steam-local-app-data'
    $steamId4 = '76561197991565051'
    $profileId4 = ([Convert]::ToUInt64($steamId4, 10)).ToString('x')
    $config4 = Join-Path $localAppData4 "4A Games\Metro 2033\$profileId4\user.cfg"
    New-Item -ItemType Directory -Path $game4,(Join-Path $steamRoot4 'config') -Force | Out-Null
    Copy-Item -LiteralPath $MetroExe -Destination (Join-Path $game4 'metro.exe')
    $loginUsers = @"
"users"
{
    "$steamId4"
...
"@
    [IO.File]::WriteAllText((Join-Path $steamRoot4 'config\loginusers.vdf'), $loginUsers, (New-Object Text.UTF8Encoding($false)))
    & (Join-Path $PackageRoot 'Install-Metro2033ReduxVR.ps1') -SteamRoot $steamRoot4 -EpicManifestRoot $null -LocalAppDataRoot $localAppData4 -NonInteractive
    Assert-True (Test-Path -LiteralPath $config4 -PathType Leaf) 'Installer did not derive the clean Steam Metro profile path.'
    Assert-Setting (Read-All $config4) 'r_quality_level' '3' 'Installer did not configure the derived Steam profile.'
