...
// Forward declaration - defined further down, but needed by
// Hooked_CreateDXGIFactory below for the Metro2033ReduxVR DXGI 1.1
// upgrade. See the comment at its use site.
extern HRESULT(__stdcall *fnOrigCreateDXGIFactory1)(REFIID riid, _Out_ void **ppFactory);
...
HRESULT __stdcall Hooked_CreateDXGIFactory(REFIID riid, void **ppFactory)
{
	...
	// Metro2033ReduxVR: satisfy this DXGI 1.0 request by actually
	// calling CreateDXGIFactory1. Metro 2033 Redux (a 2014 title) asks
	// for a plain IDXGIFactory via the legacy CreateDXGIFactory entry
	// point, which leaves DXGI in its 1.0 mode for the process. SteamVR
	// then refuses to set up a full compositor client, logging
	// "Failed to create sync texture. Ensure application was built
	// using DXGI 1.1 or later (i.e. Call CreateDXGIFactory1)" and
	// falling back to "the limited version of CVRCompositorClient" -
	// which makes every Submit fail with
	// VRCompositorError_SharedTexturesNotSupported no matter how the
	// submitted texture is created. Confirmed from SteamVR's own
	// vrclient_metro.txt; see Notes/12.
	//
	// IDXGIFactory1 derives from IDXGIFactory, and CreateDXGIFactory1
	// accepts __uuidof(IDXGIFactory), so the game still receives
	// exactly the interface it asked for and is none the wiser. Falls
	// back to the original call if this fails for any reason, so a
	// system where it doesn't work degrades to previous behaviour
	// rather than failing to start.
	HRESULT hr = E_FAIL;
	if (fnOrigCreateDXGIFactory1) {
		hr = fnOrigCreateDXGIFactory1(riid, ppFactory);
		if (SUCCEEDED(hr))
			LogInfo("  Metro2033ReduxVR: upgraded CreateDXGIFactory -> CreateDXGIFactory1 for SteamVR compositor support\n");
		else
			LogInfo("  Metro2033ReduxVR: CreateDXGIFactory1 upgrade failed (%x), falling back to CreateDXGIFactory\n", hr);
	}
	if (FAILED(hr))
		hr = fnOrigCreateDXGIFactory(riid, ppFactory);
