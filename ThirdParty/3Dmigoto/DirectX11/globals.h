...
struct Globals
{
	...
	// VR head tracking (Metro2033ReduxVR milestone 1 - rotation only).
	// Updated once per frame in HackerSwapChain::Present via VRPose.cpp.
	// Consumed in HackerContext::PatchMappedVRCameraData, which patches
	// cb_main_matrices1's m_V/m_iV rotation in place when the game
	// Unmaps it (the game binds this buffer once via VSSetConstantBuffers
	// and thereafter just Map(WRITE_DISCARD)/Unmap's its contents every
	// frame rather than rebinding - see Notes/06 in the mod project).
	// vrHeadRotation3x3 is a DELTA rotation (game-space convention, see
	// VRPose.cpp) relative to vrReferenceRotation3x3, which is captured
	// once from the raw OpenVR pose on the first valid frame - applying
	// the HMD's raw absolute orientation directly would discard the
	// game's own view direction and replace it with something tied to
	// SteamVR's arbitrary room calibration.
	bool vrHeadRotationValid;
	float vrHeadRotation3x3[9];
	bool vrReferenceRotationCaptured;
	float vrReferenceRotation3x3[9];

	// Cached from the most recent patched cb_main_matrices1 (camera)
	// update: the NEW (head-tracked) 3x4 view and the game's own
	// (unmodified) 4x4 projection. Consumed by
	// HackerContext::PatchMappedVRObjectData to recompute
	// cb_main_matrices0's per-object m_WV/m_WVP - see Notes/07 in the
	// mod project for why patching cb1 alone isn't visible: most scene
	// geometry reads a precombined WVP from cb0, not cb1's m_V directly.
	bool vrCachedViewValid;
	float vrCachedView3x4[12];
	float vrCachedProjection4x4[16];

	// Correction = V_new * V_old^-1 (row-major 3x4 affine), computed
	// alongside vrCachedView3x4 in the same cb1 patch. Some objects
	// (confirmed: the weapon viewmodel) get their "local-to-view"
	// transform pre-baked CPU-side into a per-instance vertex buffer
	// rather than reading the camera buffer at all - see
	// HackerContext::PatchMappedVRInstanceXformData and Notes/09 in the
	// mod project. Left-multiplying this correction onto an already-baked
	// local-to-OLD-view transform yields local-to-NEW-view directly,
	// without needing to separately recover the "local" part.
	float vrViewCorrection3x4[12];

	// The same mapping WITHOUT the head rotation, for the viewmodel: it puts
	// the weapon in the player's body frame so it stays where the hand is
	// when the head turns, instead of following the view.
	float vrWeaponViewCorrection3x4[12];
	bool vrWeaponViewCorrectionValid;

	// Metro2033ReduxVR milestone 2 (stereo). The game renders one eye per
	// frame, alternating - we can't make a closed-source engine draw its
	// scene twice from a wrapper, so this is the "alternate eye
	// rendering" approach (the same technique Luke Ross's R.E.A.L. mods
	// use). vrCurrentEye selects which eye the frame currently being
	// rendered belongs to; it flips once per Present.
	//   0 = left, 1 = right.
	int vrCurrentEye;

	// Per-eye projection matrices, built to match the HMD's actual
	// (asymmetric) frustums but expressed in the GAME's own convention -
	// left-handed/+Z-forward, depth [0,1], column-vector - and reusing
	// the game's own near/far so depth behaves identically. Derived from
	// vr::IVRSystem::GetProjectionRaw tangents the first time we see the
	// game's projection; see VRPose::EnsureStereoParams. Substituted for
	// the game's m_P in PatchMappedVRCameraData, which also feeds the
	// recomputed m_WVP for all the geometry we already correct.
	float vrEyeProjection4x4[2][16];

	// Matching inverse projections. Metro is a deferred renderer: its
	// lighting reconstructs view/world position from the depth buffer
	// using m_iP, so leaving m_iP as the game's original while m_P
	// became per-eye would have every lighting calculation
	// reconstructing positions from a matrix that no longer describes
	// how the depth was produced.
	float vrEyeInvProjection4x4[2][16];

	// Per-eye camera offset (half the IPD, left/right) expressed in view
	// space, in game units. Applied to the view matrix's translation.
	float vrEyeOffsetView[2][3];

	// False until the two above have been computed - the eye projections
	// can only be built once we've seen the game's own projection, so
	// early frames render with the game's unmodified one.
	bool vrStereoParamsValid;
	...
	Globals() :
		...
		vrHeadRotationValid(false),
		vrReferenceRotationCaptured(false),
		vrCachedViewValid(false),
		vrWeaponViewCorrectionValid(false),
		vrCurrentEye(0),
		vrStereoParamsValid(false),
