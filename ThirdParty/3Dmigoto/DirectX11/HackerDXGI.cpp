...
{
	// Metro rebuilds its swap chain and all its render targets whenever a
	// video setting changes. Anything we cached for the previous set is now
	// pointers to freed objects, at addresses the new ones will reuse.
	StereoTwin::ResetForNewSwapChain();
	...
	/* [in] */ UINT Flags)
{
	...
	// buffer holds this frame's final image right up until Present, and
	// is not guaranteed to afterwards (flip models may have already
	// rotated or discarded it). No-op without a headset. See Notes/11.
	// Anything still held for the second eye belongs to THIS frame. The
	// last pass of a frame never sees another render-target change, so
	// without this its draws would sit in the batch and be replayed into the
	...
	// Apply any requested stereo-mode change HERE, at the frame boundary and
	// after the second eye has been flushed. Switching mid-frame would leave
	...
	if (!(Flags & DXGI_PRESENT_TEST) && mHackerDevice) {
		...
		// Flip to the other eye so the NEXT frame the game renders is
		...
		if (kPerformanceTelemetryEnabled) {
			...
			StereoTwin::MaybeDumpInventory();
			StereoTwin::ReportFrameStats();
			StereoSinglePass::FrameTiming(
				mHackerDevice->GetPassThroughOrigDevice1(),
				mHackerDevice->GetPassThroughOrigContext1());
			ReportEyeCBCost();
	...
	if (!(Flags & DXGI_PRESENT_TEST)) {
		...
		// Metro2033ReduxVR: poll HMD orientation for the current frame,
		// consumed by HackerContext::VSSetConstantBuffers. No-op if no
		// headset/SteamVR is present.
		// Development probes are deliberately opt-in. Several of them use
		// debug-register watchpoints, cross-thread inspection, or repeated
		// process-memory scans; leaving them in the per-frame path can cause
		// severe frametime spikes and would contaminate scripted-event tests.
		static const bool kRuntimeDiagnosticsEnabled = false;
		VRPose::EnsureInitialised();
		if (kRuntimeDiagnosticsEnabled) {
			VRPose::EnsureMemoryScannerStarted();
			VRPose::RunInputInjectionTest();
			VRPose::DumpGameModuleIfRequested();
			VRPose::ArmMatrixSourceProbe();
			VRPose::ReArmMatrixSourceProbe();
			VRPose::ArmCameraMatrixProbe();
			VRPose::ArmCommandStreamProbe();
			VRPose::RunPlayerStateHunt();
		...
		// angular error during a head turn.  With Metro's narrower visibility
	...
	_Out_  void **ppSurface)
{
	...
	// Metro2033ReduxVR true stereo: twin the back buffer HERE, not at
	// Present.
	//
	// The back buffer is the one render target DXGI makes rather than
	// CreateTexture2D, so it needs registering by hand - but registering it
	// during Present was too late by a whole frame: the game creates its
	// render-target view over the back buffer during start-up, long before
	// the first Present, and a view is only twinned if its texture already
	// has a twin. The result was a back buffer with a twin but no twin
	// VIEW, so the engine's final blit was never doubled and the second
	// eye stayed black while every earlier pass rendered correctly.
	if (SUCCEEDED(hr) && Buffer == 0 && ppSurface && *ppSurface && mHackerDevice) {
		ID3D11Texture2D *tex = NULL;
		if (SUCCEEDED(((IUnknown *)*ppSurface)->QueryInterface(__uuidof(ID3D11Texture2D), (void **)&tex)) && tex) {
			StereoTwin::RegisterBackBuffer(mHackerDevice->GetPassThroughOrigDevice1(), tex);
			tex->Release();
		}
	}
