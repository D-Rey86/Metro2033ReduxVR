...
#define NVAPI_DLL L"nvapi.dll"
	...
	// Metro2033ReduxVR: disabled. This checks the NVIDIA driver profile
	// against what 3Dmigoto's own NVIDIA 3D Vision stereo path expects,
	// and if it does not match, spawns an elevated rundll32 helper to fix
	// it - the UAC "Windows host process (Rundll32)" prompt on every
	// launch. This mod does not use NVIDIA's own stereo path at all (VR
	// rendering is entirely our own, via OpenVR), so there is nothing for
	// this check to correctly fix - and in this environment the helper
	// itself fails ("module could not be found") even when approved,
	// meaning the prompt could never be satisfied by clicking Yes.
	// Renaming the game folder's nvapi64.dll does not stop it either:
	// Windows' DLL search order falls back to the driver's own copy in
	// System32, which should not be touched (other software may depend
	// on it). Skipping the call here is the actually-safe fix.
	static const bool kCheckNvProfile = false;
	if (kCheckNvProfile)
...
static const bool kLoadLibraryRedirectTraceEnabled = false;
	...
	LPCWSTR our_name, LPCWSTR library)
{
	...
	if (_wcsicmp(lpLibFileName, fullPath) == 0)
	{
		...
		if (GetModuleFileName(migoto_handle, fullPath, MAX_PATH)) {
			...
			if (kLoadLibraryRedirectTraceEnabled)
