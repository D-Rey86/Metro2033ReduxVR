...
public:
	...
	// Metro2033ReduxVR: 64-byte STAGING buffer used to read back a
	// per-instance "xform" slot before correcting it - see
	// HackerContext::PatchInstanceXformAtOffset and Notes/09.
	ID3D11Buffer *mVRInstanceXformReadbackStaging;

	// Metro2033ReduxVR: our OWN per-instance vertex buffer, bound in place of
	// the game's for weapon draws only.
	//
	// The game keeps every object's per-instance transform in one shared 1MB
	// dynamic ring buffer, and writing into it corrupts whatever else happens
	// to be nearby - proven in Notes/10 Update 5, where writing byte-for-byte
	// UNCHANGED data still froze props across the level. Merely Map/Unmapping
	// it interleaved with the game's own writes is enough. So we never touch
	// it: substitute this buffer at bind time instead, and the game's memory
	// is left completely alone.
	ID3D11Buffer *mVRWeaponPrivateVB;
	ID3D11Buffer* GetOrCreateVRInstanceXformReadbackStaging();
	ID3D11Buffer* GetOrCreateVRWeaponPrivateVB();

	// Per-mesh readback buffers, keyed by index count. Required by the async
	// readback: each mesh must have its own, or a frame-old read returns
	// whichever mesh was copied last.
	std::unordered_map<UINT, ID3D11Buffer*> mVRWeaponStaging;
	ID3D11Buffer* GetOrCreateWeaponStaging(UINT key);

	// GPU-side weapon transform: compute shader, its UAV over the private
	// vertex buffer, and the parameter buffer feeding it. Replaces the
	// per-draw readback stall entirely.
	ID3D11ComputeShader *mVRWeaponCS;
	ID3D11UnorderedAccessView *mVRWeaponUAV;
	ID3D11Buffer *mVRWeaponParamsCB;
	bool GetOrCreateVRWeaponCompute(ID3D11ComputeShader **cs,
